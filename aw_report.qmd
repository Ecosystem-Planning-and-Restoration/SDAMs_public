...
Perennial indicator taxa: `r if (!is.null(params$aqua_inv)) {params$aqua_inv}`
...
Algal cover: `r if (!is.null(params$algal)){params$algal}`

`r if (params$algal_cb == TRUE){paste0('Algae exclusively appears to have been deposited from an upstream source, and no local growth is evident.')}`
