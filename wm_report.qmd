  ...
  pdf:
    include-in-header:
      - text: |
          ...
          \cfoot{}
          \lfoot{SDAM Version 1.0 September 2024, Date of site visit: \sitedate}
    template-partials:
      - before-body.tex
params:
    stream_class: NA
...
\begin{tcolorbox}
    ...
    \vspace{2mm}
    \begin{center}
        \begin{Large} SDAM Version 1.0 Release Date: September 2024 \end{Large}
    \end{center}
    \vspace{2mm}
    \begin{center}
...
\vspace{2mm}
Mean bankfull channel width (m): `r params$bankwidth`
...
Abundance of Ephemeroptera, Plecoptera, and Trichoptera: `r if (!is.null(params$aqua_inv)) {params$aqua_inv}`

Perennial indicator taxa: `r if (!is.null(params$eph_isa)) {params$eph_isa}`
...
Slope: `r if (!is.null(params$slope)){params$slope}`%
...
Shading: `r if (!is.null(params$shade)){params$shade}`%
...
Hydrophytic plants: `r if (!is.null(params$hydro)) {params$hydro}`
...
Prevalence of upland rooted plants in streambed: `r if (!is.null(params$upland_rooted)){params$upland_rooted}`
...
Differences in vegetation: `r if (!is.null(params$veg)){params$veg}`
...
Riffle-pool sequence: `r if (!is.null(params$riff)){params$riff}`
...
Particle size or stream substrate sorting : `r if (!is.null(params$substrate)){params$substrate}`
